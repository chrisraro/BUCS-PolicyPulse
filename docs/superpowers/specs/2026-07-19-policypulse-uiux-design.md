...
**Guest-first (owner revision, 2026-07-19):** `/` is publicly viewable — guests land directly on the chat interface and can type freely. Sending is the auth gate: on send (or suggested-prompt tap) a **Sign in to send** dialog opens (email/password sign-in + BU-email sign-up, same domain rules and copy as /login); the typed message is held, and after successful sign-in it automatically pushes through. Guests see a sidebar sign-in card instead of sessions; "Ask a human" also gates behind the dialog. `/api/chat` remains auth-required server-side.
